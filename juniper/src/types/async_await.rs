use crate::{ast::Selection, value::{Object, ScalarRefValue, ScalarValue, Value}, ValuesStream, FieldError};
...
use super::base::{is_excluded, merge_key_into, async_merge_key_into, Arguments, GraphQLType};
        ...
        match *selection {
            Selection::Field(Spanning {
                ...
                if f.name.item == "__typename" {
                    ...
                    object.add_field(
                        response_name,
                        Value::Scalar(Box::pin(futures::stream::once(async {
                            typename
                        })))
                    );
                    continue;
