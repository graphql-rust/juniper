...
#[doc(hidden)]
pub struct GraphQLInterfaceTypeImplementor {
    pub interface_name: &'static str,
    pub object: GraphQLObjectTypeInfo,
}
...
#[doc(hidden)]
pub static GRAPHQL_IFACE_TYPES: Lazy<HashMap<&'static str, Vec<GraphQLObjectTypeInfo>>> =
    Lazy::new(|| {
        let mut m = <HashMap<_, Vec<GraphQLObjectTypeInfo>>>::new();
        for i in inventory::iter::<GraphQLInterfaceTypeImplementor> {
            m.entry(i.interface_name).or_default().push(i.object);
        }
        m
    });
