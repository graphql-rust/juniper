...
pub async fn playground_handler(graphql_endpoint_url: &str, subscriptions_endpoint_url: Option<&'static str>) -> Result<HttpResponse, Error> {
...
pub mod subscriptions {
    ...
    use actix::{Actor, StreamHandler, ActorContext, WrapFuture, AsyncContext};
    use actix_web_actors::ws;
    use actix_web::{HttpRequest, HttpResponse, Error, web};
    use actix_web_actors::ws::{WebsocketContext, handshake_with_protocols};
    ...
    struct GraphQLWebSocketActor<Query, Mutation,Subscription, Context, S, FunStart>  where
        S: ScalarValue + Send + Sync + 'static,
        Context: Clone + Send + Sync + 'static + std::marker::Unpin,
        Query: juniper::GraphQLTypeAsync<S, Context = Context> + Send + Sync + 'static,
        Query::TypeInfo: Send + Sync,
    ...
    impl<Query, Mutation,Subscription, Context, S, FunStart> Actor for GraphQLWebSocketActor<Query, Mutation,Subscription, Context, S, FunStart>
        where
            S: ScalarValue + Send + Sync + 'static,
            Context: Clone + Send + Sync + 'static + std::marker::Unpin,
            Query: juniper::GraphQLTypeAsync<S, Context = Context> + Send + Sync + 'static,
            ...
            Subscription::TypeInfo: Send + Sync,
            FunStart: std::marker::Unpin + FnMut(&mut Context, String) -> Result<(), String> + 'static, {
        type Context = ws::WebsocketContext<GraphQLWebSocketActor<Query, Mutation,Subscription, Context, S, FunStart>>;
    }

    impl<Query, Mutation,Subscription, Context, S, FunStart> StreamHandler<Result<ws::Message, ws::ProtocolError>> for GraphQLWebSocketActor<Query, Mutation,Subscription, Context, S, FunStart>
        where
            S: ScalarValue + Send + Sync + 'static,
            Context: Clone + Send + Sync + 'static + std::marker::Unpin,
            Query: juniper::GraphQLTypeAsync<S, Context = Context> + Send + Sync + 'static,
                    ...
                    match request.type_name.as_str() {
                        GQL_CONNECTION_INIT => {
                            match (self.on_start)(&mut self.graphql_context, String::from(m)) {
                                Ok(_) => {
                                    ctx.text(format!(r#"{{"type":"{}", "payload": null }}"#, GQL_CONNECTION_ACK));
                                    ctx.text(format!(r#"{{"type":"{}", "payload": null }}"#, GQL_CONNECTION_KEEP_ALIVE));
                                },
                                Err(_err) => {
                                    ctx.text(format!(r#"{{"type":"{}", "payload": null }}"#, GQL_CONNECTION_ERROR))
                                }
                            }
                        }
                        GQL_START => {
                            ...
                            );
                            {
                                ...
                                let actor_future = async move {
                                    ...
                                    let values_stream = {
                                        let subscribe_result = coordinator.subscribe(&graphql_request, &context).await;
...
mod tests {
    ...
    async fn graphql_post_works_json_post() {
        ...
        let req = test::TestRequest::post()
            ...
            .set_payload(r##"{ "variables": null, "query": "{ hero(episode: NEW_HOPE) { name } }" }"##)
            .uri("/")
            .to_request();
