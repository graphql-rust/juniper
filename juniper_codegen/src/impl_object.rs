...
pub fn build_subscription(args: TokenStream, body: TokenStream, is_internal: bool) -> TokenStream {
    ...
    match _impl.trait_ {
        Some((_, ref path, _)) => {
            let name = path
                ...
                .collect::<Vec<_>>()
                .join(".");
            if !(name == "GraphQLSubscription" || name == "juniper.GraphQLSubscription") {
                panic!("The impl block must implement the 'GraphQLSubscription' trait");
            }
        }
        None => {
            // panic!("The impl block must implement the 'GraphQLObject' trait");
    ...
    let name = match impl_attrs.name.as_ref() {
        Some(type_name) => type_name.clone(),
        None => {
            let error_msg = "Could not determine a name for the object type: specify one with #[juniper::subscription(name = \"SomeName\")";
    ...
    let mut definition = util::GraphQLSubscriptionDefiniton {
        name,
        _type: target_type.clone(),
        context: impl_attrs.context,
        scalar: impl_attrs.scalar,
